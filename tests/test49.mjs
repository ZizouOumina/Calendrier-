...
console.log('\n== 119) Prévu vs réalisé — Bilan ==');
{
  ...
  ok(/Lun 8,0 \/ 7,1 h/.test(p.txt) && /Mar 4,0 \/ 7,1 h/.test(p.txt) && /Mer 0,0 \/ 7,1 h/.test(p.txt), 'jour par jour : Lun 8/7,1, Mar 4/7,1, Mer 0/7,1');
  ok(/Jeu prévu 7,1 h/.test(p.txt) && /Dim prévu 4,3 h/.test(p.txt), 'les jours à venir montrent le prévu (jeu 7,1 h, dim 4,3 h)');
  ok(/Révision 9,0 \/ 13,0 h/.test(p.txt) && /Projets 3,0 \/ 8,3 h/.test(p.txt), 'totaux par type sur les jours passés (rév. 9/13, proj. 3/8,3)');
