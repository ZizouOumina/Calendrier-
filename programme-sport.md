...
- **Montée en charge : deux semaines à la moitié des tours, puis le volume complet.** La Batcave affiche directement le bon nombre de tours, il n'y a rien à calculer. Semaines 1 et 2 (14 → 27 septembre) : moitié. À partir de la semaine 3 (28 septembre) : programme entier. Le passage double le volume d'un coup — si les courbatures font sauter une séance cette semaine-là, c'est le moment d'utiliser la réduction de charge, pas de serrer les dents.
- **La première séance, le lundi 14, est une séance de référence.** On ne suit pas la séance normale : on mesure le point de départ. Après l'échauffement, une seule série à l'échec sur quatre mouvements — tractions, dips, pompes, dead hang — notée dans l'onglet Sport. Puis on s'arrête là pour le haut du corps ; le cou et les avant-bras se font en douceur. Sans ce point de départ, la surcharge progressive part du bas de la fourchette et on ne saura jamais ce qu'on valait au début.
- **Pesée le lundi, à jeun**, au réveil, après les toilettes, avant de boire. Une fois par semaine et pas tous les jours : un poids se lit sur une tendance, et se peser quotidiennement fait prendre des décisions sur du bruit.
- **Photos du corps un dimanche sur deux** (27 septembre, 11 octobre, et ainsi de suite) : face, profil, dos, mêmes lieu, même lumière, mêmes poses, deux minutes. Le mètre ruban dit le centimètre, la photo dit la forme — et c'est elle qu'on croit quand la balance ne bouge pas.
...
- Pas d'historique : cible = tours × bas de fourchette.
- Toutes les séries de la dernière séance au haut de la fourchette (ou au-dessus) : on monte d'un cran, jamais deux, et on repart du bas. Retomber dans le bas est voulu.
- Sinon : chaque série vise la dernière plus une répétition (plus 5 s pour les exercices en secondes), plafonnée au haut de la fourchette.
- Les exercices à durée fixe (« 15-20 min ») n'ont pas de progression.

### Les crans, par famille
- **Poussée horizontale** : pompes inclinées barre haute → barre basse → au sol → déclinées → diamant → archer.
- **Poussée verticale** : pike pieds au sol → sur barre basse → sur banc → contre un poteau.
- **Jambes** : poids du corps → pied arrière surélevé → + sac → + pied avant surélevé 10 cm → tempo 3 s → pause 2 s en bas → shrimp squat → pistol.

### Le lest
Tractions à 12 reps propres sur toutes les séries, dips à 15, jambes dès que tu dépasses proprement le haut de la fourchette : +2,5 kg, une seule fois. Tu retombes à 5-8 reps, tu remontes vers 12, tu rajoutes 2,5 kg. Le cou fait exception : +1,25 kg, jamais 2,5.

### Sans traction ni dip
- Tractions : rows australiens pieds surélevés 10-15, puis négatives 5 s (5-8).
- Dips : maintien bras tendus 20-40 s, puis négatives 5 s (5-8), puis dips pieds au sol 8-15.
...
- Objectif à six mois : 64 → 70 kg avec le plan d'alimentation (voir programme-alimentation.md).
