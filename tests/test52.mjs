...
/* mardi 8 sept 06:30 : Sport 05:00, Douche 06:00 et Coran 06:15 déjà commencés ; il reste 15 blocs sur 18 ; mercredi : 18 */
...
console.log('\n== 171) Activer : un événement par bloc, aujourd\'hui (restants) et demain ==');
{
  ...
  ok(crees.length === 33, '33 événements créés : 15 pour mardi (3 blocs déjà commencés) + 18 pour mercredi (' + crees.length + ')');
  ...
  ok(r8 && Object.keys(r8).length === 15 && r9 && Object.keys(r9).length === 18 && Object.values(r8).every(x => x.id && x.empreinte && x.cree === true), 'relevés locaux : 15 + 18 blocs avec id, empreinte et marque « créé par la Batcave »');
  const opt = await local(fr, 'batcave-gcal-ecriture');
  ok(opt && opt.actif === true && opt.bilan && opt.bilan.crees === 33 && opt.bilan.passes === 3, 'option active, bilan : 33 créés, 3 déjà commencés non envoyés');
  ...
  ok(crees2.length === 33, '« Pousser maintenant » ne recrée rien (' + crees2.length + ')');
  const det = await fr.evaluate(() => document.getElementById('gcal-detail').textContent);
  ok(/33 inchangés/.test(det), 'détail : ' + det.slice(0, 90));
  ...
  ok(crees3.length === 33 && r9b && Object.keys(r9b).length === 18 && Object.values(r9b).every(x => x.cree === false), 'sans relevé local, les 18 événements de demain sont reconnus et adoptés (marqués « pas créés par la Batcave »), aucun doublon');
  const opt2 = await local(fr, 'batcave-gcal-ecriture');
  ok(opt2.bilan.adoptes === 18, 'bilan : 18 adoptés');
  /* désactiver retire les rappels envoyés */
  await fr.evaluate(() => document.getElementById('gcal-toggle').click());
  await page.waitForTimeout(500);
  const sup = await appels(fr, 'delete_event');
  ok(sup.length === 15, 'désactiver ne retire que ce que la Batcave a créé : les 15 de mardi (ceux de mercredi, adoptés au tour précédent, restent) — ' + sup.length);
...
console.log('\n== 172) Option active au démarrage : envoi automatique ; bloc déplacé → mise à jour ==');
{
  ...
  const crees = await appels(fr, 'create_event');
  ok(crees.length === 32, 'les 32 autres sont créés au démarrage (' + crees.length + ')');
...
console.log('\n== 172b) Ton agenda porte déjà le planning en événements récurrents : adoptés, jamais doublés ==');
{
  ...
  ok(crees.length === 30, '30 créations seulement (33 − 3 adoptés)');
  const r8 = await local(fr, 'batcave-gcal-2026-09-08');
  ok(r8 && r8['p4'] && r8['p4'].id === 'rec1_20260908' && r8['p4'].cree === false, 'le relevé pointe sur l\'instance récurrente adoptée, marquée « pas créée par la Batcave »');
  /* désactiver : les 3 événements adoptés (les tiens) restent, seuls les 30 créés partent */
  await fr.evaluate(() => document.getElementById('gcal-toggle').click());
  await page.waitForTimeout(700);
  const sup = await appels(fr, 'delete_event');
  ok(sup.length === 30 && !sup.some(d => /^rec/.test(d.input.eventId)), 'désactiver retire les 30 créés et jamais tes événements adoptés (' + sup.length + ')');
  await ctx.close();
}
