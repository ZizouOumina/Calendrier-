...
const MI_SEPT = '2026-09-23T10:00:00+02:00';   /* premier jour du programme (mercredi 23, decale le soir du 22) */
...
console.log('\n== 112) Semis : un seul palier, le mois ==');
{
  ...
  /* La grille ne prevoit rien avant le jour 1 : la cible de septembre part de la, pas du
     ...
     porte donc plus que huit jours de programme, et sa cible de revision tombe de 40,7 a
     35,5 h. Elle n'est jamais ecrite a la main : c'est ce que la grille contient sur le
     mois, a 89 %. */
  ok(m9 && m9.cible === 35.5, 'révision de septembre calculée depuis la grille, à partir du 23 : 35,5 h (' + (m9 && m9.cible) + ')');
...
console.log('\n== 113) Comparaison sur des données réelles ==');
{
     ...
     « trop tot pour juger » (ecoules <= 1), et elle doit rester visible ici. */
  {
    ...
    ok(r1 && /attendu 2,8 h/.test(r1.txt), 'au matin du jour 1, la grille n\'attend que 2,8 h — les jours d\'avant ne comptent plus (' + (r1 && (r1.txt.match(/attendu [^ ]+ h/) || [''])[0]) + ')');
    /* Ce que ce bloc garde, c'est qu'AUCUN objectif n'est en RETARD au matin du jour 1 :
       ...
       un faux verdict : le decalage du depart au 23 a ramene la cible du mois a 35,5 h,
       soit moins que les 40 h deja journalisees par la graine. Un objectif atteint est un
       troisieme etat legitime. Seul le retard serait faux ici. */
    ok(r1 && !/retard/.test(r1.led) && s1 && !/retard/.test(s1.led), 'jour 1 : aucun retard, pas de faux verdict — REV[' + (r1 && r1.led) + '] SEANCES[' + (s1 && s1.led) + ']');
    await j1.ctx.close();
  }
  ...
  /* 40 h ont bien été travaillées du 1er au 10 : le RÉEL les garde. L'ATTENTE, elle, ne
     compte que le programme : 35,5 h de cible × la part de septembre écoulée = 11,8 h
     (le programme court du 23 au 30, et le 25 à 10:00 en a consommé une bonne part).
     Une séance faite avant le départ compte, mais ne crée pas de retard. */
  ok(rev && /réel 40,0 h/.test(rev.txt) && /attendu 11,8 h/.test(rev.txt) && /avance|atteint/.test(rev.led), 'Révision : réel 40,0 h pour 11,8 h attendues au 25 à 10:00 → en avance : ' + (rev && rev.txt.slice(0, 80)));
...
console.log('\n== 114) Éditer une cible, supprimer, ajouter ==');
{
  ...
  ok(rev && /attendu 20,0 h/.test(rev.txt) && /avance/.test(rev.led), 'recalcul immédiat sur une cible de 60 h : attendu 20,0 h, 40 h réelles → en avance (' + (rev && rev.txt.slice(0, 70)) + ')');
...
console.log('\n== 116) Tableau de bord : objectifs du mois ==');
{
  ...
  ok(/Révision 40,0 \/ 35,5 h/.test(d.txt), 'ligne compacte réel / cible : ' + d.txt.slice(0, 60));
