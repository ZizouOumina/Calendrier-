...
ok(/Révision2h30\/5h15/.test(d.cells[0]), 'révision avec objectif du jour : ' + d.cells[0]);
ok(/Projetsperso1h30/.test(d.cells[1]), 'projets perso du jour : ' + d.cells[1]);
  ...
  ok(d.barres === 2 && d.largeur === '48%', 'deux jauges (révision et projets), révision à 48 % de 5 h 15 (mercredi, Anki matinal) (' + d.largeur + ')');
  await ctx.close();
}
{
  ...
  ok(/Révision0\/5h15/.test(d.cours), 'objectif tout de même rappelé : ' + d.cours);
  await ctx.close();
}
{
  ...
  ok(/1h\/5h15/.test(d.cours), 'après un bloc de cours : ' + d.cours);
...
}
{
  ...
  ok(/Révision0/.test(cells[0]), 'la révision reste à zéro : ' + cells[0]);
  ok(/Projetsperso1h/.test(cells[1]), 'le temps va bien aux projets : ' + cells[1]);
  await ctx.close();
}
