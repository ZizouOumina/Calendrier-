...
console.log('\n== 240) Sport : mardi 22 septembre = semaine 1 (moitié des tours), mardi 20 octobre = retest ==');
{
  const { ctx, fr } = await ouvrir('2026-09-22T06:30:00+02:00');
  const r = await fr.evaluate(() => ({ debut: window.__bcProgrammeDebut, note: document.getElementById('programme-note').textContent }));
  ok(r.debut === '2026-09-22', 'PROGRAMME_DEBUT = 2026-09-22 (' + r.debut + ')');
  ...
  ok(/démarre le/.test(n2) && /22/.test(n2), 'avant le 22 : « démarre le 22 sept. » (' + n2 + ')');
  await s.ctx.close();
  /* La semaine de programme court du mardi au lundi depuis le 22. Le mardi n'est PAS
     off au sport : premierJourSport() rend le 22 lui-meme, et le retest tombe au meme jour
     de semaine -- la semaine 5 court du 20 au 26 octobre, son retest tombe le mardi 20. */
  const r5 = await ouvrir('2026-10-20T06:30:00+02:00');
  const n5 = await r5.fr.evaluate(() => document.getElementById('programme-note').textContent);
  ok(/semaine 5/.test(n5) && /mardi : retest/.test(n5), 'mardi 20 octobre : semaine 5, retest (' + n5 + ')');
  await r5.ctx.close();
}
