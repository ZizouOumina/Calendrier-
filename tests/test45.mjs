...
console.log('\n== 108) Deux blocs de révision échus sans session : signalés, avec le déficit ==');
{
  ...
  ok(/Bloc manqué — 4 h 20 de révision non faites \(Anki matinal 05:30, Anki 1 07:20, Anki 2 08:20, Étudier en avance 09:20\)/.test(t), 'texte : « 4 h 20 de révision non faites » — ' + ((t.match(/Bloc manqué[^\n]*/) || [''])[0]));
...
console.log('\n== 109) Le lendemain, la cible intègre le report ==');
{
  ...
  ok(/\/ 7 h 05/.test(cells[0]) && /\+2 h/.test(cells[0]), 'révision : « / 7 h 05 +2 h ↪ » (jeudi) (obtenu ' + cells[0] + ')');
  ...
  ok(/Révision — 0 sur 7 h 05/.test(plan), 'Plan du jour : cible 7 h 05 (5 h 05 + 2 h de report)');
  await ctx.close();
}
{
  ...
  const cells = await fr.evaluate(() => [...document.querySelectorAll('#dash-temps .temps-cell .tv')].map(e => e.innerText.replace(/\s+/g,' ')));
  ok(/\/ 5 h 05/.test(cells[0]) && !/\+/.test(cells[0]), 'report périmé ignoré : « / 5 h 05 »');
  await ctx.close();
}
...
}
{
  ...
  const { ctx, fr } = await ouvrir(MERCREDI_MIDI, {'batcave-sessions': s});
  const t = await texte(fr, '#dash-plan');
  ok(/2 h 50 de révision non faites \(Anki matinal 05:30, Étudier en avance 09:20\)/.test(t), 'Anki 1 et 2 couverts ; déficit réel 2 h 50 — ' + ((t.match(/Bloc manqué[^\n]*/) || [''])[0]));
  await ctx.close();
}
