...
console.log('\n== 171) Activer : un événement par bloc, aujourd\'hui (restants) et demain ==');
{
  ...
  ok(crees.length === 108, '108 événements créés : mardi et mercredi, plus les 9 oct., 12 oct. et 8 déc. (' + crees.length + ')');
  ...
  ok(r8 && Object.keys(r8).length === 21 && r9 && Object.keys(r9).length === 21 && Object.values(r8).every(x => x.id && x.empreinte && x.cree === true), 'relevés locaux : 21 + 21 blocs avec id, empreinte et marque « créé par la Batcave »');
  const opt = await local(fr, 'batcave-gcal-ecriture');
  ok(opt && opt.actif === true && opt.bilan && opt.bilan.crees === 108 && opt.bilan.passes === 1, 'option active, bilan : 108 créés, 1 déjà commencé non envoyé');
  ...
  ok(crees2.length === 108, '« Pousser maintenant » ne recrée rien (' + crees2.length + ')');
  const det = await fr.evaluate(() => document.getElementById('gcal-detail').textContent);
  ok(/108 inchangés/.test(det), 'détail : ' + det.slice(0, 90));
  ...
  ok(crees3.length === 108 && r9b && Object.keys(r9b).length === 21 && Object.values(r9b).every(x => x.cree === false), 'sans relevé local, les 21 événements de demain sont reconnus et adoptés (marqués « pas créés par la Batcave »), aucun doublon');
  ...
  ok(sup.length === 87, 'désactiver ne retire que ce que la Batcave a créé, sur TOUS les jours poussés (108 − les 21 adoptés du mercredi) — ' + sup.length);
...
console.log('\n== 172) Option active au démarrage : envoi automatique ; bloc déplacé → mise à jour ==');
{
  ...
  const crees = await appels(fr, 'create_event');
  ok(crees.length === 107, 'les 107 autres sont créés au démarrage (' + crees.length + ')');
...
console.log('\n== 172b) Ton agenda porte déjà le planning en événements récurrents : adoptés, jamais doublés ==');
{
  ...
  ok(crees.length === 105, '105 créations seulement (108 − 3 adoptés)');
  ...
  const sup = await appels(fr, 'delete_event');
  ok(sup.length === 105 && !sup.some(d => /^rec/.test(d.input.eventId)), 'désactiver retire les 105 créés et jamais tes événements adoptés (' + sup.length + ')');
  await ctx.close();
}
