...
console.log('\n== 108) Deux blocs de révision échus sans session : signalés, avec le déficit ==');
{
  ...
  ok(/Bloc manqué — 4,3 h de révision non faites \(Anki matinal 05:30, Anki 1 07:20, Anki 2 08:20, Cartes du dernier cours 09:20, Annales 10:20\)/.test(t), 'texte : « 4,3 h de révision non faites (Anki 1 07:20, Anki 2 08:20, Cartes du dernier cours 09:20, Annales 10:20) »');
...
console.log('\n== 109) Le lendemain, la cible intègre le report ==');
{
  ...
  ok(/\/ 6,4h/.test(cells[0]) && /\+2h/.test(cells[0]), 'révision : « / 6,4h +2h ↪ » (jeudi) (obtenu ' + cells[0] + ')');
  ok(/\/ 2,7h/.test(cells[1]) && !/\+/.test(cells[1]), 'projets : inchangé (obtenu ' + cells[1] + ')');
...
}
{
  ...
  const cells = await fr.evaluate(() => [...document.querySelectorAll('#dash-temps .temps-cell .tv')].map(e => e.innerText.replace(/\s+/g,' ')));
  ok(/\/ 4,4h/.test(cells[0]) && !/\+/.test(cells[0]), 'report périmé ignoré : « / 4,4h »');
  await ctx.close();
}
...
}
{
  ...
  const { ctx, fr } = await ouvrir(MERCREDI_MIDI, {'batcave-sessions': s});
  const t = await texte(fr, '#dash-plan');
  ok(/2,8 h de révision non faites \(Anki matinal 05:30, Cartes du dernier cours 09:20, Annales 10:20\)/.test(t), 'Anki 1 et 2 couverts ; déficit réel 2,8 h sur Cartes et Annales');
  await ctx.close();
}
