...
console.log('\n== 190) Un Pomodoro de révision se voit sur six pages ==');
{
  ...
  ok(/Aujourd'hui[\s\S]*?25 min/.test(await txt('#rev-stats')), 'Études : aujourd\'hui 25 min');
  ...
  await aller('bilan');
  ok(/Révision[\s\S]*?25 min/.test(await txt('#bilan-grid')), 'Bilan : révision 25 min cette semaine');
...
console.log('\n== 191) Un Pomodoro de projet perso se voit sur Business, le tableau de bord et l\'agenda ==');
{
  ...
  ok(/Aujourd'hui[\s\S]*?25 min/.test(await txt('#proj-stats')), 'Business : aujourd\'hui 25 min');
  ok(/Shopify[\s\S]*?25 min/.test(await txt('#proj-by-name')), 'Business : par projet, Shopify 25 min');
  await aller('dashboard');
  ok(/Projets perso[\s\S]*?25 min/.test(await txt('#dash-temps')), 'Tableau de bord : projets 25 min');
  ok(/50 min au total/.test(await txt('#dash-temps-total')), 'Score du jour : 50 min au total');
  await aller('agenda');
  ok(/50 min · 2 blocs/.test(await txt('#ag-jtotal')), 'Agenda Batcave : 50 min · 2 blocs');
...
console.log('\n== 195) Séries de sport → Sport, tableau de bord, Objectifs ==');
{
  ...
  ok(/Bas complet[\s\S]*?1\/6/.test(await txt('#dash-sport')), 'Tableau de bord : Bas complet 1/6 (crunch lesté et cou latéral en fin de séance)');
}
