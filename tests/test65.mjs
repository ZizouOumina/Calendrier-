...
console.log('\n== 240) Sport : mercredi 23 septembre = semaine 1 (moitié des tours), jeudi 22 octobre = retest ==');
{
  const { ctx, fr } = await ouvrir('2026-09-23T06:30:00+02:00');
  const r = await fr.evaluate(() => ({ debut: window.__bcProgrammeDebut, note: document.getElementById('programme-note').textContent }));
  ok(r.debut === '2026-09-23', 'PROGRAMME_DEBUT = 2026-09-23 (' + r.debut + ')');
  ...
  ok(/démarre le/.test(n2) && /23/.test(n2), 'avant le 23 : « démarre le 23 sept. » (' + n2 + ')');
  await s.ctx.close();
  /* La semaine de programme court du mercredi au mardi depuis le 23. Le mercredi EST off
     au sport : premierJourSport() avance au jeudi 24, et le retest tombe au meme jour de
     semaine -- la semaine 5 court du 21 au 27 octobre, son retest tombe le jeudi 22. */
  const r5 = await ouvrir('2026-10-22T06:30:00+02:00');
  const n5 = await r5.fr.evaluate(() => document.getElementById('programme-note').textContent);
  ok(/semaine 5/.test(n5) && /jeudi : retest/.test(n5), 'jeudi 22 octobre : semaine 5, retest (' + n5 + ')');
  await r5.ctx.close();
}
