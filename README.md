...
- `tests/test80.mjs` — le lot 31, sur données fictives : Alfred qui répond aux QUESTIONS (« où j'en suis ? », « et après ? », « combien de cartes ? ») sans rien cocher ni rien lancer, la séance de sport comptée à la fraction de ses cases, la boucle poids → calories qui ne se laisse plus retourner par une seule pesée, et le mode partiels annoncé une semaine avant de s'enclencher.
- `tests/test84.mjs` — Alfred v2 : la phrase part seule une seconde après le dernier mot (plus d'Entrée), la conversation continue après chaque réponse, la grammaire couvre la journée entière (journal, sport, Anki, tâches, navigation) à deux fautes de dictée près, et ce qui n'est pas compris revient sous forme de proposition.
...
- `tests/test83.mjs` — Alfred quand le micro s'ouvre vraiment (le cas du fichier local sur Mac) : le réacteur s'allume aussi sur ce chemin-là, la phrase s'écrit mot à mot pendant qu'on parle, la réponse s'affiche puis se dit. Et le cas inverse, le cadre qui refuse le micro (la page publiée), où la dictée du clavier prend le relais.
