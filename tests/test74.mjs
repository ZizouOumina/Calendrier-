...
console.log('\n== 308) La copie vierge : revenir exactement au 19, premier jour du programme ==');
{
  const { ctx, fr, page } = await ouvrir('2026-09-19T07:00:00+02:00');
  const item = await fr.evaluate(() => [...document.querySelectorAll('#dash-plan li')].map(l => l.innerText).join(' | '));
  ok(/Copie vierge à prendre/.test(item), 'le 19, le plan du jour la réclame avant la première saisie');

  await fr.evaluate(() => document.querySelector('[data-plan-vierge]').click());
  await page.waitForTimeout(400);
  const v = await local(fr, 'batcave-copie-vierge');
  ok(v && v.date === '2026-09-19' && v.n > 0, 'copie prise : ' + (v && v.n) + ' entrées, état du ' + (v && v.date));
...
console.log('\n== 309) La copie vierge survit à un nouveau départ ==');
{
  const { ctx, fr } = await ouvrir('2026-11-02T09:00:00+02:00',
    {'batcave-copie-vierge': {date:'2026-09-19', pris:'2026-09-19T07:00:00.000Z', n: 3, data:{'batcave-poids':[{date:'2026-09-19', valeur:64}]}}});
  ...
  ok(/19 sept/.test(etat) && /état du premier jour/.test(etat), 'le panneau reconnaît une copie prise le premier jour (' + etat.slice(0, 80) + ')');
...
console.log('\n== 310) Les rappels d\'agenda n\'écrivent rien avant le 15 ==');
{
  ...
  /* le 19, premier jour du programme, ils partent */
  const ap = await ouvrir('2026-09-19T04:30:00+02:00', {'batcave-gcal-ecriture': {actif:true, depuis:'2026-09-09'}}, true);
  ...
  ok(creesApres > 20, 'le 19 au matin, les rappels partent (' + creesApres + ' événements)');
  await ap.ctx.close();
}
