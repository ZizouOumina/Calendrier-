...
/* Le mois de l'argent (onglet Budget). Le virement de 1 400 EUR arrive le 28 ; le mois
   de l'argent va donc du 28 au 27. Les courses se font le samedi, et une periode en compte
   4 ou 5 : le montant pose le 28 doit tenir meme avec 5, le reste part le 27. Ce test
   verifie les chiffres, le fil (solde a zero le 27, jamais negatif), le recalcul quand une
   charge change, et la mise en page sur iPhone. */
...
console.log('— vendredi 25 septembre : la premiere periode, 28 sept. -> 27 oct.');
{
  ...
  ok(a.cagnotte === 150, 'le 28 : 150 EUR dans la cagnotte (' + a.cagnotte + ')');
  ok(a.periode.debut === '2026-09-28' && a.periode.fin === '2026-10-27', 'periode du 28 sept. au 27 oct.');
  ok(a.periode.samedis === 4, '4 samedis de courses');
  ok(pres(a.periode.balayage, 58.71), 'le 27 : environ 58,71 EUR a balayer (' + a.periode.balayage.toFixed(2) + ')');
  ok(pres(a.moyenne, 191.69), 'epargne moyenne : 191,69 EUR par mois (' + a.moyenne.toFixed(2) + ')');
  ok(a.prochaine5 && a.prochaine5.debut === '2026-11-28' && a.prochaine5.samedis === 5, 'prochaine periode a 5 samedis : 28 nov. -> 27 dec.');
  ok(pres(a.prochaine5.balayage, 7.64), 'avec 5 samedis, il reste encore 7,64 EUR le 27');
  const L = a.lignes;
  ok(L[0].montant === 1400 && L[0].plus, 'le fil commence par le virement');
  ok(L[1].montant === 100 && /club/.test(L[1].action), 'puis le club, 100 EUR, le 28');
  ok(L[2].montant === 150 && L[2].cagnotte, 'puis la cagnotte, le 28');
  ...
  ok(Math.abs(L[L.length - 1].solde) < 0.001, 'le 27, le compte est a zero apres le balayage');
  ...
  ok(/28 sept\..*27 oct\..*4 samedis/.test(txt), 'la periode et ses samedis sont ecrits en tete');
...
console.log('— jeudi 3 decembre : une periode a 5 samedis');
{
  const {ctx, fr} = await ouvrir('2026-12-03T20:00:00+01:00');
  const a = await fr.evaluate(() => window.__bcMoisArgent('2026-12-03'));
  ok(a.periode.debut === '2026-11-28' && a.periode.samedis === 5, 'periode du 28 nov., 5 samedis');
  ok(a.cagnotte === 150, 'la cagnotte du 28 ne bouge pas : 150 EUR');
  ok(a.periode.balayage >= 0, 'et le compte ne passe pas dans le rouge (' + a.periode.balayage.toFixed(2) + ' EUR le 27)');
  const tete = await fr.evaluate(() => document.getElementById('ma-periode').textContent);
  ok(/28 nov\..*27 déc\..*5 samedis/.test(tete), 'la tete dit 5 samedis (' + tete + ')');
  ...
  ok(neg.length === 0, '24 periodes : aucun 27 dans le rouge ' + JSON.stringify(neg));
  ...
  ok(await fr.evaluate(() => window.__bcMoisArgent('2027-01-27').periode.debut === '2026-12-28' && window.__bcMoisArgent('2027-01-28').periode.debut === '2027-01-28'), 'le 27 appartient a la periode d\'avant, le 28 ouvre la suivante');
  await ctx.close();
}
