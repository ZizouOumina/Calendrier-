...
console.log('\n== 250) Journée exceptionnelle : TP jeudi 24 sept. 11:00 → 13:00 ==');
{
  ...
  ok(lundi.includes('20:30 Clore le cours du jour'), 'un autre jour reste tel quel');
  ...
  ok(/TP · Anatomía I/.test(liste) && /Sacrifié ce jour-là : .*Question ouverte ou autre/.test(liste) && /aujourd'hui/.test(liste), 'panneau : la journée, ses blocs sacrifiés (' + liste.slice(0, 120) + ')');
  await ctx.close();
}
...
console.log('\n== 254) Pomodoro ferme : lanceur ouvert seul au début d\'un bloc, rappel orange ensuite ==');
{
  ...
  await c2.close();
  const { ctx: c3, fr: f3 } = await ouvrir('2026-09-15T13:02:00+02:00');   /* bloc Español · gramática */
