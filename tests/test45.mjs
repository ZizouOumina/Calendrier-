...
console.log('\n== 108) Deux blocs de révision échus sans session : signalés, avec le déficit ==');
{
  ...
  ok(/Bloc manqué — 3 h 30 de révision non faites \(Anki 1 07:20, Anki 2 08:20, Étudier en avance 09:20\)/.test(t), 'texte : « 3 h 30 de révision non faites » — ' + ((t.match(/Bloc manqué[^\n]*/) || [''])[0]));
...
}
{
  ...
  const { ctx, fr } = await ouvrir(MERCREDI_MIDI, {'batcave-sessions': s});
  const t = await texte(fr, '#dash-plan');
  ok(/2 h de révision non faites \(Étudier en avance 09:20\)/.test(t), 'Anki 1 et 2 couverts ; déficit réel 2 h — ' + ((t.match(/Bloc manqué[^\n]*/) || [''])[0]));
  await ctx.close();
}
