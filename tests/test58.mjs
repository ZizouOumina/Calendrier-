...
const JOURS = [['lundi','2026-08-31','Sport','lundi (cours 17:30 \u2192 19:30)'],['mardi','2026-09-01','Sport','mardi (cours 15:30 \u2192 17:30)'],['mercredi','2026-09-02','Anki matinal','mercredi (cours 15:30 \u2192 19:30)'],['jeudi','2026-09-03','Sport','jeudi (cours 15:30 \u2192 19:30)'],['vendredi','2026-09-04','Projets perso matinal',"vendredi (Jumu'ah \u00b7 cours 15:30 \u2192 19:30)"],['samedi','2026-09-05','Sport','samedi (courses)'],['dimanche','2026-09-06','__06:30 Douche + préparation','dimanche']];
for(const [nom, iso, premier, libelle] of JOURS){
  ...
  ok(JSON.stringify(r.semaine) === JSON.stringify(['Sport','Sport','Anki matinal','Sport','Projets perso matinal','Sport','Douche + préparation']), nom + ' : vue de la semaine, 05:30 de lundi à dimanche = ' + r.semaine.join(' · '));
  await ctx.close();
}
await browser.close();
console.log(errs ? '\n' + errs + ' ÉCHEC(S)' : '\nTOUT VERT');
