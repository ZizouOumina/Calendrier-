...
console.log('\n== 199) Liste de courses finie le dimanche → coche du samedi 5, série 2 ==');
{
  ...
  ok(/^17\/17 articles/.test(await txt('#courses-summary')), 'Courses : 17/17 articles (' + await txt('#courses-summary') + ')');
...
console.log('\n== 200) Samedi : « Fait aujourd\'hui », le tableau de bord la liste ==');
{
  ...
  await aller('courses');
  ok(/^17\/17 articles/.test(await txt('#courses-summary')), 'Courses : cocher l\'habitude remplit la liste');
  await aller('habitudes');
