/* Lot 45 (23 septembre, nuit) : ce que ce lot a change, et qui pourrait se defaire sans bruit.
   ...
   3. Le samedi dort : lever 06:30, la muscu passe a 18:30 derriere la course.
...
console.log('\n== 261) Le cran de charge suit l\'exercice ==');
{
  const LOG = {'batcave-sport-log': [
    ...
    log('Curl inversé lesté', 'Haut lourd', [20,20,20], 2),
  ...
  const c = await fr.evaluate(() => {
    ...
    const r = {}; ['Flexion du cou','Extension du cou','Tractions','Élévations latérales','Crunch lesté','Curl inversé lesté','Dips'].forEach(n => { const x = window.__bcCibleProchaine(ex[n]); r[n] = {charge: x.charge, monter: x.monter, series: x.series}; });
    return r;
  });
  ...
  ok(c['Curl inversé lesté'].charge === 3, 'curl inversé à 2 kg → 3 kg (' + c['Curl inversé lesté'].charge + ')');
...
console.log('\n== 262) Les élévations latérales, lundi et samedi ==');
{
  ...
  ok(s['Haut lourd'].noms.includes('Élévations latérales') && s['Bras · épaules · mollets'].noms.includes('Élévations latérales'), 'présentes le lundi et le samedi');
  ok(!s['Haut volume'].noms.includes('Élévations latérales') && !s['Bas complet'].noms.includes('Élévations latérales'), 'et seulement ces deux jours');
  ...
  ok(s['Haut lourd'].duree === '57 min' && s['Bras · épaules · mollets'].duree === '53 min', 'durées annoncées : 57 et 53 min');
  await ctx.close();
}

console.log('\n== 263) Le samedi dort une heure de plus ==');
{
  ...
  ok(r.lever === '06:30' && r.g[0] === '06:30 Douche + préparation', 'lever 06:30, comme le dimanche');
  ok(r.g.includes('18:00 🏃 Course à pied') && r.g.includes('18:30 Sport') && r.g.includes('19:30 Dîner'), 'course 18:00, muscu 18:30, dîner 19:30');
  ok(r.g.filter(x => / Sport$/.test(x)).length === 1, 'une seule séance de muscu dans la journée');
  ok(r.sport === 'Bras · épaules · mollets' && r.prevu.sport === 1, 'la séance du samedi est toujours comptée');
  ok(Math.abs(r.som - (24 - 21 - 35/60 + 6.5)) < 0.01, 'nuit du vendredi : 21:35 → 06:30 = 8 h 55 (' + r.som.toFixed(2) + ' h)');
  ok(r.g.includes('09:20 Approfondir') && r.g.some(x => /^16:00 Cartes d'erreurs/.test(x)), 'aucun bloc de travail n\'a bougé');
  const c = await fr.evaluate(() => [window.__bcConsigne('Sport', 6, '2026-10-03'), window.__bcConsigne('Sport', 1, '2026-10-05')]);
  ok(/18:00/.test(c[0]) && !/kilomètre/.test(c[0]), 'la consigne du samedi parle de la course de 18:00, pas du kilomètre du matin');
