...
console.log('\n== 112) Semis : trimestres et mois ==');
{
  ...
  ok(t1 && t1.cible === 380 && m9 && m9.cible === 127, 'T1 révision 380 h → septembre 127 h');
...
console.log('\n== 113) Comparaison sur des données réelles ==');
{
  ...
  ok(rev && /réel 40,0 h/.test(rev.txt) && /attendu 63,5 h/.test(rev.txt) && /retard/.test(rev.led), 'Révision : réel 40,0 h, attendu 63,5 h (50 % de 127) → en retard : ' + (rev && rev.txt.slice(0, 80)));
  ok(rev && /à ce rythme : 80,0 h/.test(rev.txt), 'projection à ce rythme : 80,0 h à la fin');
  const som = await ligne(fr, 'Sommeil');
  ok(som && /réel 7,50 h/.test(som.txt) && /atteint/.test(som.led), 'Sommeil moyen 7,50 h ≥ 7,25 → atteint');
...
console.log('\n== 116) Tableau de bord : objectifs du mois ==');
{
  ...
  ok(/Révision 40,0 \/ 127,0 h/.test(d.txt), 'ligne compacte réel / cible : ' + d.txt.slice(0, 60));
