...
console.log('\n== 312) Le réacteur annonce le jour 1, pas un « 0 % » ==');
{
  ...
  const { ctx, fr, page } = await ouvrir('2026-09-23T06:45:00+02:00');
  ...
  /* 173 et non 174 : le depart a glisse au 23 septembre, et le 14 mars ne bouge pas --
     c'est la fin de la saison, pas une duree a reporter. */
  ok(v.num === '1' && v.unite === '/173', 'le premier matin affiche « 1/173 » — 23 sept. → 14 mars, pas « 0 % » (' + v.num + v.unite + ')');
...
}
{
  ...
  const { ctx, fr } = await ouvrir('2026-09-23T12:00:00+02:00');
  const t = await fr.evaluate(() => document.getElementById('dash-plan').innerText);
